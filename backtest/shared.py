...
CONFIG_PATH = ROOT / "parameter" / "backtesting_parameter_v49.json"
WORKER_STATE_PATH = ROOT / "backtest" / "runtime_state.json"
EXECUTOR_STATE_PATH = ROOT / "backtest" / "executor_state.json"
HCM_TZ = pytz.timezone("Asia/Ho_Chi_Minh") if pytz else ZoneInfo("Asia/Ho_Chi_Minh")
...
POINT_VALUE = int(STRATEGY.get("point_value", 1))
...
def get_recovery_signal(rsi_history, current_price=None, reference_price=None, lookback=7):
    ...
    previous_window = rsi_history[-(lookback + 1):-1]
    deep_oversold_bars = count_deep_bars(previous_window, RSI_DEEP_LONG, "below", lookback=lookback)
    deep_overbought_bars = count_deep_bars(previous_window, RSI_DEEP_SHORT, "above", lookback=lookback)
    price_up = (
...
def build_signal(close_price, reference_close, rsi_history, adx_value, scalp_signal, scalp_atr_value):
    prev_rsi = rsi_history[-2] if len(rsi_history) >= 2 else None
